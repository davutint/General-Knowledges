...
public class questionmanager2 : MonoBehaviour
{
    ...
    private void SetQuestion()
    {
        ...
        gameManager.ıpucuobj.GetComponent<Image>().sprite = questions[currentQuestion].ıpucu;
        ...
        for (int i = 0; i < buttonTexts.Length; i++)
        {
            ...
            buttons[i].image.color = new Color32(43,50,255,184);
        ...
        if (AnswerIndex == correctAnswerIndex) //RIGHT ANSWER
        {
            ...
            if (currentQuestion == questions.Length)// Sertifika burada gösteriliyor..Ses buraya eklenecek 
            {
                ...
                Sertifika.GetComponent<Image>().DOFade(1, .7f).OnComplete(() =>
                {
                    ...
                    gamemanager2.Instance.OyuncuIsmi.DOFade(1, 2f);
            ...
            SliderArttır();
            buttons[correctAnswerIndex].image.color = Color.blue;//new Color32(0, 0, 175, 255);
    ...
    void IpUcuControl()
    {
        ...
        gamemanager2.Instance.IpUcuObjPanel.transform.DOLocalMove(new Vector3(-2000f, 0, 0), .5f);
    }

    void SliderArttır()
    {
        gamemanager2.Instance.slider.value += ArttırılacakMiktar;
    }
    ...
    {
        yield return new WaitForSeconds(.7f);
        gamemanager2.Instance.DogruyuGoster();
    ...
    void Tuslarıpasifyap()
    {

    }
    void Tuslarıaktifyap()
    {
        for (int i = 0; i < buttons.Length; i++)
        {
            buttons[i].interactable = true;
