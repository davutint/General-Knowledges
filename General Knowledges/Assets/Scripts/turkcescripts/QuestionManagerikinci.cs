...
using System;
public class QuestionManagerikinci: MonoBehaviour
{
    public GameManagerikinci gameManager;
    ...
    public void SetQuestion()
    {
        ...
        for (int i = 0; i < buttonTexts.Length; i++)
        {
            ...
            buttons[i].image.color = new Color32(221, 64, 213, 185);      //butonların rengini her soruda burada sıfırlıyorum
    ...
    public void AnswerButton(int AnswerIndex)
    {
        ...
        if (AnswerIndex == correctAnswerIndex) //RIGHT ANSWER
        {
            ...
            if (currentQuestion == questions.Length)// Sertifika burada gösteriliyor..Ses buraya eklenecek
            {
                BolumSonuSesi();
                Sertifika.GetComponent<Image>().DOFade(1, .7f).OnComplete(() =>
                {
                    AltinYildiz.transform.DOLocalMove(new Vector3(0, 1250, 0), 1f);
                    GameManagerikinci.Instance.OyuncuIsmi.DOFade(1, 2f);
            ...
            SliderArttır();
            buttons[correctAnswerIndex].image.color = new Color32(150, 0, 255, 185);
    ...
    void IpUcuControl()
    {
        ...
        GameManagerikinci.Instance.IpUcuObjPanel.transform.DOLocalMove(new Vector3(-2000f, 0, 0), .5f);
    }

    void SliderArttır()
    {
        GameManagerikinci.Instance.slider.value += ArttırılacakMiktar;
    }
    ...
    {
        yield return new WaitForSeconds(.7f);
        GameManagerikinci.Instance.DogruyuGoster();
        for (int i = 0; i < buttons.Length; i++) //butonları aktif hale getiriyor
        {
            buttons[i].interactable = true;
        }
